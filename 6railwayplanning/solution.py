...
def BFS(graph,k):
    ...
    limit = int(2**k)
    for bottleneck in range(limit,0,-1):
        queue = deque()
        ...
        while queue:
            ...
            for edge in neighbours:
                if edge.cnode.id not in visited and edge.is_available(bn_cap=bottleneck):
...
def get_k(graph):
    ...
    k_power = int(math.log(C)/math.log(2))
    return k_power
